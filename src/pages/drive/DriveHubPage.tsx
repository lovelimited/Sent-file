        ...
        <div>
          <div className="flex items-center gap-2 mb-1">
            ...
            >
              ...
              <span>{isAdmin ? 'โหมดผู้ดูแลระบบ: แสดงทรัพยากรทั้งหมด' : 'คลังส่วนตัว: แสดงเฉพาะของตนเอง'}</span>
            </span>
          </div>
          <h1 className="text-2xl font-bold tracking-tight text-slate-900 flex items-center gap-2.5">
            <FolderOpen className="h-6 w-6 text-amber-600" />
            <span>{isAdmin ? 'คลัง Google Drive & ทรัพยากรโรงเรียน' : 'คลัง Google Drive ของฉัน'}</span>
          </h1>
          <p className="text-xs sm:text-sm text-slate-500 mt-1">
            {isAdmin
              ? 'ศูนย์รวมโฟลเดอร์กลาง โฟลเดอร์กลุ่มสาระฯ และเอกสารทางการทั้งหมดของโรงเรียน (แอดมินมองเห็นของทุกคน)'
              : 'ศูนย์รวมโฟลเดอร์และลิงก์เอกสารส่วนตัวของคุณ (สามารถมองเห็นและจัดการได้เฉพาะตนเอง)'}
          </p>
        </div>
      ...
      <div className="flex flex-col md:flex-row gap-3 justify-between items-stretch md:items-center">
        ...
        <div className="relative w-full md:w-80">
          ...
          <input
            ...
            className="w-full rounded-xl border border-slate-200 bg-white pl-10 pr-4 py-2 text-xs text-slate-900 placeholder-slate-400 focus:border-blue-600 focus:outline-none shadow-2xs"
          />
        </div>

        {/* Category Tabs */}
        <div className="flex flex-wrap gap-1.5 border border-slate-200 rounded-xl p-1 bg-white text-xs shadow-2xs">
          <button
      ...
      {isLoading ? (
        ...
        <div className="rounded-2xl border border-slate-200 bg-white p-12 text-center shadow-xs">
          ...
          <p className="text-xs text-slate-500 mt-1">
            {isAdmin
              ? 'ลองเปลี่ยนคำค้นหาหรือกดปุ่ม "เพิ่มทรัพยากร"'
              : 'กดปุ่ม "เพิ่มทรัพยากร" เพื่อบันทึกโฟลเดอร์ Google Drive หรือลิงก์เอกสารส่วนตัวของคุณ'}
          </p>
        </div>
      ) : (
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
          {filteredResources.map((item) => {
            ...
            return (
              <div
                key={item.id}
                className="rounded-2xl border border-slate-200 bg-white p-5 flex flex-col justify-between hover:border-blue-300 hover:shadow-md transition-all shadow-xs group"
              >
                <div>
                ...
                <div className="mt-5 pt-3 border-t border-slate-100 flex items-center justify-between">
                  ...
                  <a
                    ...
                    className="inline-flex items-center gap-1.5 rounded-lg bg-blue-50 border border-blue-200 px-3 py-1.5 text-xs font-semibold text-blue-700 hover:bg-blue-600 hover:text-white transition-all shadow-2xs"
                  >
          ...
          <div className="w-full max-w-lg rounded-2xl border border-slate-200 bg-white p-6 shadow-2xl animate-in zoom-in-95 duration-150">
            <div className="flex items-center justify-between border-b border-slate-200 pb-4 mb-4">
              <h2 className="text-lg font-bold text-slate-900 flex items-center gap-2">
                ...
                <span>{isAdmin ? 'เพิ่มทรัพยากร / โฟลเดอร์โรงเรียน' : 'เพิ่มทรัพยากร / โฟลเดอร์ส่วนตัว'}</span>
              </h2>
              <button
            ...
            <form onSubmit={handleAddSubmit} className="space-y-4">
              ...
              <div>
                <label className="block text-xs font-medium text-slate-700 mb-1">
                  ชื่อทรัพยากร / เอกสาร <span className="text-red-500">*</span>
                </label>
                <input
                  type="text"
                  required
